...
const LiveTv = ({ onNavigate }) => {
  ...
  const { current, next, nextStartMins, upcoming } = epgState;
      ...
      <section className="live-tv-hero-stage">
        <div ref={playerContainerRef} className={`live-player-container ${isFullscreen ? 'is-fullscreen' : ''}`}>
          ...
          <div className="live-player-top-bar">
            <div className="live-status-pill"><span className="live-pulse-dot"></span><span className="live-text">LIVE</span></div>
        ...
        <div className="live-meta-container">
          ...
          {next && (
            <div className="live-next-row">
              ...
              <div className="next-time">{minsTo12h(nextStartMins)}</div>
            ...
            {current && (
              <div className="channel-card active">
                <div className="channel-img-wrap">
                  <img src={current.image} alt={current.title} className="channel-img"/>
                  <span className="channel-badge live">🔴 {current.live ? 'LIVE' : 'ON AIR'}</span>
                </div>
                <div className="channel-info">
                  <span className="channel-cat">{current.category}</span>
                  <h4 className="channel-title">{current.title.toUpperCase()}</h4>
                  <p className="channel-desc">{minsTo12h(slotToMins(current.time))} IST{next ? ` · Next: ${next.title}` : ''}</p>
                </div>
              </div>
            )}
            {upcoming.map((slot, i) => (
              <div className="channel-card" key={i}>
                <div className="channel-img-wrap">
                  <img src={slot.image} alt={slot.title} className="channel-img"/>
                  <span className={`channel-badge ${slot.live ? 'live' : 'upcoming'}`}>
                    {slot.live ? '🔴 LIVE' : '⏰ UPCOMING'}
                  </span>
                </div>
                <div className="channel-info">
                  <span className="channel-cat">{slot.category}</span>
                  <h4 className="channel-title">{slot.title.toUpperCase()}</h4>
                  <p className="channel-desc">{minsTo12h(slotToMins(slot.time))} IST</p>
                </div>
              </div>
            ))}
          </div>
